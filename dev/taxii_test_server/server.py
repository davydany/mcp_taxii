...
def apply_filters(query, added_after: str | None, match_id: str | None, 
                  match_type: str | None, match_version: str | None):
...
# TAXII 2.0 Endpoints
@app.get("/taxii2/", response_model=Discovery20Response)
async def discovery_20(db: Session = Depends(get_db)):
    """TAXII 2.0 Discovery endpoint."""
    discovery = db.query(Discovery).first()
    if not discovery:
...
@app.get("/taxii2/{api_root}/collections/", response_model=list[Collection20])
...
@app.get("/taxii2/{api_root}/collections/{collection_id}/", response_model=Collection20)
async def get_collection_20(
    ...
    collection = db.query(Collection).filter(
        and_(Collection.id == collection_id, Collection.api_root_id == api_root)
    ).first()
...
async def get_objects_20(
    ...
    query = db.query(STIXObject).join(
        collection_objects,
        STIXObject.id == collection_objects.c.object_id
    ...
    return JSONResponse(
        ...
        media_type="application/taxii+json;version=2.0",
    )


@app.post("/taxii2/{api_root}/collections/{collection_id}/objects/", response_model=Status20)
...
async def get_manifest_20(
    ...
    # Query objects with filters
    query = db.query(STIXObject, collection_objects.c.date_added).join(
        collection_objects,
        STIXObject.id == collection_objects.c.object_id
    ).filter(collection_objects.c.collection_id == collection_id)
...
# TAXII 2.1 Endpoints
@app.get("/taxii21/", response_model=Discovery21Response)
...
@app.get("/taxii21/{api_root}/collections/", response_model=list[Collection21])
...
@app.get("/taxii21/{api_root}/collections/{collection_id}/", response_model=Collection21)
...
@app.get("/taxii21/{api_root}/collections/{collection_id}/objects/", response_model=Envelope21)
...
@app.post("/taxii21/{api_root}/collections/{collection_id}/objects/", response_model=Status21)
...
@app.get("/taxii21/{api_root}/collections/{collection_id}/manifest/", response_model=ManifestEnvelope21)
...
# Status endpoints
@app.get("/taxii2/{api_root}/status/{status_id}/", response_model=Status20)
...
@app.get("/taxii21/{api_root}/status/{status_id}/", response_model=Status21)
